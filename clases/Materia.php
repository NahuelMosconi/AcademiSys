...
class Materia
{
    ...
    /**
     * DELETE lógico con control de integridad.
     * Devuelve [exito, mensaje]. NO permite la baja si la materia tiene alumnos
     * inscriptos activos en alguna de sus comisiones.
     */
    public function darDeBaja(int $id): array
    {
        // Primero verificamos que no haya inscriptos activos en comisiones de esta materia.
        $chk = $this->db->prepare(
        ...
        if ((int)$chk->fetchColumn() > 0) {
            return [false, 'No se puede dar de baja: la materia tiene alumnos inscriptos activos.'];
        }
        // Si no hay inscriptos, recién ahí la marcamos inactiva.
        $stmt = $this->db->prepare("UPDATE Materia SET activo = 0 WHERE id_materia = :id");
        $stmt->execute(['id' => $id]);
        return [true, 'Materia dada de baja.'];
    }
}
